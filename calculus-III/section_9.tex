...
\begin{proof}
	\enewline
	\begin{itemize}
		\item[1.] Всем точкам $\cO$ сопоставим ячейку \textbf{требуемого типа} 
			с рациональными параметрами. Таких ячеек не более чем счетное множество:
\[
...
\begin{theorem}(Свойства меры Лебега)
    \begin{itemize}
        \item[2.] Все открытые и замкнутые множества измеримы, потому
			что открытые множества представляются в виде счетного
			объединения ячеек, а замкнутые --- дополнения открытых.
...
\begin{proof}
	...
	из последнего свойства меры Лебега. Имеем
\[
	F_{\frac{1}{n}} \subseteq A \subseteq G_{\frac{1}{n}}
\]
	И
\[
	\forall n~ \l(C \setminus B) < \l(G_{\frac{1}{n}} \setminus F_{\frac{1}{n}}) < \frac{2}{n}
\]
	Поэтому $\l(C \setminus B) = 0$. $B$, $C$ борелевские, потому что являются пересечением 
...
\end{lemma}
\begin{proof}
	Пусть $F_{\frac{1}{n}}$ --- замкнутое множество, удовлетворяющее 
\[
	\l(F_{\frac{1}{n}} \setminus A) < \frac{1}{n}
\]
	Надо избавиться от его неограниченности. Вспомним, что
	это множество совершенно не нужно целым. Тогда разобъем его:
\[
	K_{\frac{1}{n},~ i} = \overline{B(0, i)} \cap F_{\frac{1}{n}}
\]
	Каждое $K$ замкнуто как пересечение двух замкнутых и ограничено как пересечение
	чего-то с ограниченным множеством. Поэтому $K$ --- компакты. Осталось 
	наобъединять этих компактов для всех $n$ так, чтобы мера остатка была равна нулю.
	Такое уже делали:
\[
	\bigcup_{n \in \bN}{\bigcup_{i}{K_{\frac{1}{n},~ i}}}
\]
\end{proof}

\begin{lemma}
    $T \colon \Rm \to \Rn \in C(\Rm)$, $\forall E \in \mathfrak{M}~ \lambda(E) = 0
    \Lra \lambda(T(E)) = 0$, \\ тогда $\forall A \in \mathfrak{M}^m~ T(A) \in \mathfrak{M}^n$.
\end{lemma}
\begin{proof}
...
\begin{figure}[ht]
    \centering
    \incfig{cione}
    \caption{Измеримость при гладком отображении}
\end{figure}
