...
\begin{proof}
    \begin{align*}
        ...
        &= \{\, \elv = [\der{f}{\x}]\h + r(\h) \,\} \\
        &= g(f(\x) + \elv) = g(f(\x)) + [\der{g}{f(\x)}]\elv + \widetilde{r}(\elv) \\
        &= g(f(\x)) + [\der{g}{f(\x)}][\der{f}{\x}]\h + [\der{g}{f(\x)}]r(\h) +
        \widetilde{r}(\elv) \\
    \end{align*}
    Осталось показать, что $[\der{g}{f(\x)}]r(\h) + \widetilde{r}(\elv) =
    o(\norm{\h})$.
\[
    \norm{[\der{g}{f(\x)}]r(\h) + \widetilde{r}(\elv)} \leqslant
...
\begin{proof}
    ...
    $g = f(\ela + t(\elb - \ela))$ для $t \in [0, 1]$ --- дифференцируемо как
