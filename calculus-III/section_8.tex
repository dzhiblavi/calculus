...
\begin{align*}
    \a_c &= \inf_{\a \in [a, b]}{\{\,\a \mid \im{\gamma\big|_{[\a, c]}}
            \subseteq B_c\,\}} \\
    \b_c &= \sup_{\b \in [a, b]}{\{\,\b \mid \im{\gamma\big|_{[c, \b]}}
            \subseteq B_c\,\}}
\end{align*}
    Тогда $\forall c~ \im{\gamma\big|_{(\a_c, \b_c)}} \subseteq B_c$. Кроме того:
\[
...
\begin{lemma}
	Локально постоянное отображение связного множества постоянно на нём.
\end{lemma}
\begin{proof}
	Зафиксируем произвольную точку $a$ из области определения. Положим
\[
	A = \{\,x \mid f(x) = f(a) \,\}
\]
	Это множество открыто потому, что $f$ локально постоянно. С другой
	стороны, по той же самой причине дополнение $A$ тоже открыто. 
	Раз так, одно из этих множеств
	обязано быть пустым, так как иначе область определения не окажется
	связной. Раз $a \in A$, пустым окажется дополнение $A$, что и требовалось.
\end{proof}
